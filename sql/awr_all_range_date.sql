...
define DEFAULT_OUTPUT_FILENAME = 'awr_all_range_out.sql'
define NO_ADDM = 0
...
--
select 'Interval  : ' || extract(hour from snap_interval) || ' HR : ' || extract(minute from snap_interval) || ' MIN' from dba_hist_wr_control
union
select 'Retention : ' || extract(day from retention) from dba_hist_wr_control;

-- enter the date range
select 'Please enter dates in DD-MON-YY HH24 format:' from dual;
...
CURSOR c_snapshots IS
 select inst_num, start_snap_id, end_snap_id, begin_interval_time,end_interval_time
 from (
 select s.instance_number as inst_num,
 s.snap_id as start_snap_id,begin_interval_time,end_interval_time,
 ...
 s.begin_interval_time >=to_date('&BEGIN_DATE','DD-MON-YY HH24')
 and  s.begin_interval_time <=to_date('&END_DATE','DD-MON-YY HH24')  + (1/1440*5)
 )
...
select d.dbid into c_dbid from v$database d;
