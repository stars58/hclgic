...
define DEFAULT_OUTPUT_FILENAME = 'addm_all_range_out.sql'
define NO_ADDM = 0
...
dbms_output.put_line('');
 dbms_output.put_line('prompt Beginning ADDM Generation...');
...
-- Iterate through snapshots
 ...
 v_awr_reportname := 'addmrpt_'||cr_snapshot.inst_num||'_'||cr_snapshot.start_snap_id||'_'||cr_snapshot.end_snap_id||v_report_suffix;
 
dbms_output.put_line('prompt Creating ADDM Report '||v_awr_reportname
...
-- Disable terminal output to stop AWR text appearing on screen
 dbms_output.put_line('set termout off');
     ...
     descr := 'ADDM run: snapshots [' || cr_snapshot.start_snap_id || ', '
              || cr_snapshot.end_snap_id || '], instance ' || cr_snapshot.inst_num
              || ', database id ' || c_dbid;
     ...
     -- set time window
     dbms_advisor.set_task_parameter(name, 'START_SNAPSHOT',cr_snapshot.start_snap_id);
     dbms_advisor.set_task_parameter(name, 'END_SNAPSHOT', cr_snapshot.end_snap_id);

     -- set instance number
     dbms_advisor.set_task_parameter(name, 'INSTANCE', cr_snapshot.inst_num);

     -- set dbid
     dbms_advisor.set_task_parameter(name, 'DB_ID', c_dbid);
 ...
 dbms_output.put_line('select dbms_advisor.get_task_report('''||:task_name||''', ''TEXT'', ''TYPICAL'') from   sys.dual;');
